...
# ---- 1) Fit model on your last_year data ----
# Uses: y = tariff_lin, x = manuf_gdp_lin
stopifnot(all(c("tariff_lin","manuf_gdp_lin") %in% names(last_year)))
model  <- lm(tariff_lin ~ manuf_gdp_lin, data = last_year)
sm     <- summary(model)

# ---- 2) Build stats in Excel-style terms ----
n      <- sm$df[1] + sm$df[2] + 1L      # observations
p      <- 1L                            # number of predictors (excluding intercept)
df_reg <- p
df_res <- n - (p + 1L)
df_tot <- n - 1L
...
multipleR <- sqrt(R2)                   # Excel shows positive sqrt(R^2)
SE_reg <- sm$sigma                      # residual standard error

# Coefficients table (Intercept and manuf_gdp_lin)
...
# ---- 3A) Create a new Excel workbook that matches your template layout ----
wb <- createWorkbook()
...
# ANOVA block (df, SS, MS, F, Significance F)
...
# Coefficients block
writeData(wb, 1, "", startCol = 1, startRow = 20) # spacer
coef_tbl <- coef_df
names(coef_tbl)[1] <- ""  # blank header cell like your screenshot
writeData(wb, 1, coef_tbl, startCol = 1, startRow = 21, colNames = TRUE)

# Number formatting
num_fmt <- createStyle(numFmt = "0.0000")
addStyle(wb, 1, num_fmt, rows = 6:9, cols = 2, gridExpand = TRUE)                    # stats
addStyle(wb, 1, num_fmt, rows = 15:(14+nrow(anova_tbl)), cols = 3:6, gridExpand = TRUE) # anova
addStyle(wb, 1, num_fmt, rows = 22:(21+nrow(coef_tbl)), cols = 2:7, gridExpand = TRUE)  # coefs
